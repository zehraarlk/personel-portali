...
export async function fetchVideos(kategori = '') {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.detail ||
        `Videolar alınamadı (${response.status})`
    );
  }
...
export async function fetchSizdenGelenler(
  kategori
) {
  const qs = kategori
    ? `?kategori=${encodeURIComponent(kategori)}`
  ...
  const response = await fetch(
    `${API_BASE}/sizden-gelenler/${qs}`
  );

  if (!response.ok) {
    throw new Error(
      'Sizden gelenler alınamadı!'
    );
  }
...
export async function goruntulenmeArttir(id) {
  const response = await fetch(
    `${API_BASE}/sizden-gelenler/${id}/goruntule/`,
    {
      method: 'POST',
    }
  );
...
export async function fetchDuyurular(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.detail ||
        `Duyurular alınamadı (${response.status})`
    );
  }
 ...
 * { dokumanlar: [...], toplam }
 */
export async function fetchDokumanlar(q = '') {
  const qs = q
    ? `?q=${encodeURIComponent(q)}`
    : '';

  const response = await fetch(
    `${API_BASE}/dokumanlar/${qs}`
  );
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.detail ||
        `Dokümanlar alınamadı (${response.status})`
    );
  }
 ...
 * }
 */
export async function fetchMevzuatlar(
  q = '',
  altKategori = ''
) {
  ...
  if (altKategori) {
    params.set('alt_kategori', altKategori);
  }
